...
async def request(domain: str, path: str, referer: str = "", data: FormData | dict = None):
    await _request(f"http://{domain}/{path}", auth = LOGGER_AUTH, headers = {"Referer": f"http://{domain}/{referer}"}, data = (data if isinstance(data, FormData) else FormData(data)) if data else None)
